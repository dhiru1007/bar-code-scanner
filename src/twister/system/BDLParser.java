 ...
 * AB: 04-JAN-17: Added Verbose keyword
*/
/*
* 
*
...
public class BDLParser   {
    ...
    */
    ...
    protected void init() {
       ...
       cmdTab.put("LOG.LEVEL",new CmdVerbose());
    }
    ...
    public static void main(String args[]) throws Exception {
        ...
        //System.out.println("04-Jane-2017: www.BonBiz.in ");
/**
...
*/
    ...
    class CmdVerbose implements Command {
       public void exec(StreamTokenizer tok, Hashtable ctx) throws Exception {      
       
            if(tok.nextToken() == tok.TT_WORD) {
               Level curLev = log.getLevel();
               int before = (curLev == null)? 1: curLev.intValue();
               
                log.setLevel(Level.parse(tok.sval));
                int after = log.getLevel().intValue();
                log("INFO; PROC=CmdVerbose; LINE="+lineCnt+"; ID=001;  MSG=INFO: Verbose Level Before = "+before+" After =" +after);
            } else
               log("ERROR; PROC=CmdVerbose; LINE="+lineCnt+"; ID=002;  MSG=INFO: Verbose Level NOT SET"); 
    ...
    class CmdObject implements Command {
       public void exec(StreamTokenizer tok, Hashtable ctx) throws Exception {
          ...
          //       ^
          if(tok.nextToken() != tok.TT_WORD)
             log("ERROR: PROC=BDLParser.CmdObject; LINE="+lineCnt+"; ID=105;  MSG=An Object reference name expected;");
          else {
            String ref = tok.sval;
            ...
            if(tok.nextToken() == tok.TT_WORD) {
             ...
             log("ERROR: PROC=BDLParser.CmdObject; LINE="+lineCnt+"; ID=105;  MSG=An Object Class Name expected for["+ref+"];");
          }
       }
    }
       ...
       private void processRval() throws Exception {
          ...
          if(!beanMan.assignObject(curWith,setPat, param)) {
			 ...
			 if((ref == null) && beanMan.assignString(curWith,setPat,rhs)) {
			     verbose("INFO: PROC=BDLParser.CmdSet; LINE="+lineCnt+"; ID=1109;  MSG=Assigned value using ["+setPat+"] method;",3);
             }
             else if((ref == null) && beanMan.assignProperty(curWith,lhs,rhs))
                 verbose("INFO: PROC=BDLParser.CmdSet; LINE="+lineCnt+"; ID=1110;  MSG=Assigned value using ["+setPat+"] method;",3);
             else
               log("ERROR: PROC=BDLParser.CmdSet; LINE="+lineCnt+"; ID=1111;  MSG=;");
          }
          else
            verbose("INFO: PROC=BDLParser.CmdSet; LINE="+lineCnt+"; ID=1112;  MSG=Assigned value using ["+setPat+"] method;",3);
